...
- `spotify-player.json` / `spotify-audio.json` — Spotify mode only. The first is librespot's last player event (`track_changed`/`playing`/`end_of_track`/…, written by `docker/spotify/librespot-event.sh` via `--onevent`) and is how the controller learns a commanded track actually started; the second is `blank.detect`'s silent/audio verdict on the live feed. Both fail open (`broadcast/spotify-player.ts`). The controller → mixer direction in this mode is **telnet**, not a file: `spotify_track {json}` inserts the boundary + metadata, `spotify_gap on|off` declares intentional silence.
...
- **The music backend is a `MusicSource` behind one facade.** Callers import `music/source.js` (most still under the historical `subsonic` alias — a one-line import flip per file, kept that way for upstream rebases), never `music/subsonic.js` directly; the only exceptions are the two routes that configure Navidrome itself (`routes/onboarding.ts`, `routes/settings/core.ts`). `settings.music.source` selects the active source (`music/sources/registry.ts`, default `subsonic`, unknown values repair to it on load), and each source declares what it serves in `music/sources/capabilities.ts` — the facade returns a neutral empty for an optional method the source lacks, and picker tools gate on `ctx.sourceCaps` so the model is never offered a dead tool. Adding a source is one adapter under `music/sources/`, one registry line, one `MUSIC_SOURCES` entry and one capability row; **never** a `source ===` branch at a call site. Names, paths and the settings key deliberately mirror upstream PR #843. → [`docs/internals/music.md`](docs/internals/music.md)
...
- **On a live-transport source (Spotify) SUB/WAVE is still the authority that decides what plays.** The transport (`music/sources/spotify/transport.ts`) commands every track, confirms it from the player's own events, publishes it to the mixer, and never enables autoplay; with no pick ready it plays a random POOL track and says so, exactly as `auto.m3u` covers a file-mode slot. Playback truth comes from librespot's events, never from polling the Web API for position. `queue.drainToLiquidsoap` hands items to the registered `PlaybackTransport` and skips every file-only step; the queue imports no source. → [`docs/internals/broadcast.md`](docs/internals/broadcast.md)
